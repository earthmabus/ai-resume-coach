...
def test_dev_tfvars_requires_external_regional_certificates():
    text = (ROOT / "infra/global-api-routing.dev.tfvars").read_text()
    ...
    assert 'route53_public_zone_id = "Z074065915GNYUCPJUVGS"' in text
...
def test_global_edge_deployment_file_is_not_auto_loaded():
    assert (ROOT / "infra/global-api-routing.dev.tfvars").exists()
    assert not (ROOT / "infra/global-api-routing.auto.tfvars").exists()
